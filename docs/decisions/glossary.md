...
| **语义键 (Semantic Color Keys)** | 颜色配置统一为 `background / border / text / accent` 四个语义键 + 组件特有键（如 heatmap 的 `levels` 渐变数组）。`accent` = 强调色，默认跟随黑白灰单色体系，用户可改。 |
...
| **API 命名空间 (API Namespace)** | API 按分组命名：`tessera.core.card`、`tessera.core.heatmap`、`tessera.core.progressbar`。初期开发阶段，**破坏性变更直接执行，不留旧别名**。 |
